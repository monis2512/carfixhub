...
@RestController @RequestMapping("/api/queries")
public class CarQueryController {
 private final CarQueryRepository repo;
 public CarQueryController(CarQueryRepository repo){this.repo=repo;}
 @PostMapping public ResponseEntity<?> create(@RequestBody CreateQueryRequest r){
  ...
  CarQuery q=new CarQuery(); q.setCarModel(r.getCarModel().trim()); q.setServiceType(r.getServiceType().trim());
  q.setDescription(r.getDescription().trim()); q.setMobileNumber(mobile);
  q.setPhotoName(r.getPhotoName()==null?"":r.getPhotoName().trim()); q.setCreatedAt(LocalDateTime.now());
